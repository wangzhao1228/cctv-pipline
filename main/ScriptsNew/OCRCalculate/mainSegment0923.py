...
from defect_infoSegment0923 import DefectInfoProcessor, DamageScorer, PipelineDefectEvaluator, AudioAnalyzer
from video_track_obbSample0923 import VideoTracker
from frame_ocr_interpolate0923 import FrameOCR
...
if __name__ == "__main__":
    ...
    # 定义不同缺陷的置信度阈值
    conf_thresholds = {
            'QN':0.6, 'YW': 0.6, 'GL': 0.8, 'FS': 0.8, 'CJ':0.6, 'ZJ': 0.8, 'ZG':0.6,
            'LGL':0.6, 'MGL':0.6, 'SGL':0.6, 'SSGL':0.6, 'LFS':0.6, 'MFS':0.6,
            'SFS':0.6, 'SSFS':0.6, 'SZ':0.6, 'FM':0.6, 'TL':0.6, 'WZ':0.6,
            'PS':0.6, 'ZC': 0.8
        }
    ...
    pre_interpolation_data, post_interpolation_data = frame_ocr_processor.process_video(all_frame_data)  # 更新已有数据
    ...
    save_all_frame_data_to_json(pre_interpolation_data, ocr_before_json)
    ocr_after_json = "D:/workspaceTech/ultralytics/main/res/0525/0525ocr_after_json.json"
    save_all_frame_data_to_json(post_interpolation_data, ocr_after_json)
