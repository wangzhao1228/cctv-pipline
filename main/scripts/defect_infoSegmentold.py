...
class DefectInfoProcessor:
        ...
        for defect_type, defects in temp_defects.items():
            for defect_id, frames in defects.items():
                if defect_type in self.functional_defects:
                    ...
                    if len(frames) >= 3:  # and self.is_continuous(frames, max_gap=10, segment_gap=3):  # 更宽松的连续性检查
                        valid_defects[defect_type][defect_id] = frames

        # 打印出valid_defects的内容
        print("Valid Defects:")
        for defect_type, defects in valid_defects.items():
            print(f"Defect Type: {defect_type}")
            for defect_id, frames in defects.items():
                print(f"  Defect ID: {defect_id}, Frames: {frames}")

        self.update_all_frame_data(all_frame_data, valid_defects)


    def update_all_frame_data(self, all_frame_data, valid_defects):
        for frame_id, frame_data in all_frame_data.items():
    ...
    def build_defect_infos(self, current_frame_id):
        ...
        for key, info in self.active_defects.items():
            # 检查是否达到了自定义的结束条件
            if self.is_defect_ended(key, info, current_frame_id):
    ...
    def is_defect_ended(self, key, info, current_frame_id):
        """
...
class PipelineDefectEvaluator:
    ...
    def resolve_overlaps(self, defects):
        ...
        for current in defects[1:]:
            ...
            if current['起始位置'] <= last['结束位置']:
                if current['类型'] != last['类型']:
                    if self.severity_score(current['严重程度']) > self.severity_score(last['严重程度']):
                        last['结束位置'] = min(last['结束位置'], current['起始位置'])
                    else:
                        current['起始位置'] = max(last['结束位置'], current['起始位置'])
    ...
    def merge_selective_defects(self):
        ...
        sorted_defects = sorted((d for d in self.defect_infos if d['类型'] in target_defect_types),
                                key=lambda x: (x['类型'], x['严重程度'], x['起始位置']))
        ...
        # 解决重叠问题
        merged_defects = self.resolve_overlaps(merged_defects)

        # 添加未涉及的其他类型缺陷
        merged_defects.extend(d for d in self.defect_infos if d['类型'] not in target_defect_types)
